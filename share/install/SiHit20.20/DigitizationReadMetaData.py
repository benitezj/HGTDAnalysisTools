...
def buildDict(inputtype, inputfile):
    ...
    logDigitizationReadMetadata.info("Jose : SimulatedDetectors = %s", metadatadict['SimulatedDetectors'])
    logDigitizationReadMetadata.info("Jose : eventdata_items = %s", f.infos['eventdata_items'])
    ...
    logDigitizationReadMetadata.info("Jose : SimulatedDetectors after hack = %s", metadatadict['SimulatedDetectors'])
    ...
    if Nkvp==0 :
        ...
        if 'SimulatedDetectors' not in metadatadict.keys():
            if 'eventdata_items' in f.infos.keys():
                ...
                metadatadict['SimulatedDetectors'] = ['pixel','SCT','TRT','BCM','HGTD','Lucid','LAr','Tile','MDT','CSC','TGC','RPC','Truth']
...
def signalMetaDataCheck(metadatadict):
    ...
    if not skipCheck('SimulatedDetectors'):
        if 'SimulatedDetectors' in simkeys:
            ...
            possibleSubDetectors=['pixel','SCT','TRT','BCM','HGTD','Lucid','ZDC','ALFA','AFP','FwdRegion','LAr','Tile','MDT','CSC','TGC','RPC','Micromegas','sTGC','Truth']
