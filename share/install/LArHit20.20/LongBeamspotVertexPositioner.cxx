    ...
    m_randomEngineName("VERTEX")
{
    ...
    declareProperty( "TimeWidth"               , m_timeWidth                );
    declareProperty( "TimeType"                , m_timeType                 );
}
...
//    double time_offset = func.GetRandom();
    ...
    if (m_timeType == ""){
	    time_offset = CLHEP::RandGaussZiggurat::shoot( m_randomEngine , vertexSmearing->z()/Gaudi::Units::c_light , bunch_length_z/Gaudi::Units::c_light );
    }
    else if (m_timeType == "GAUSS"){
	    time_offset = CLHEP::RandGaussZiggurat::shoot( m_randomEngine , 0., m_timeWidth);
    }
    else if (m_timeType == "UNIFORM"){
	    time_offset = CLHEP::RandFlat::shoot( m_randomEngine , -m_timeWidth, m_timeWidth);
    }
