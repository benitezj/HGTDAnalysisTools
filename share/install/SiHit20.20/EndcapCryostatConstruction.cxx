...
GeoFullPhysVol* LArGeo::EndcapCryostatConstruction::createEnvelope(bool bPos)
{
  ...
  for(unsigned int ind=0; ind<cryoMotherPlanes.size(); ind++) 
  {
    ...
    else 
    {
      ...
      double myPlane, myRmin, myRmax;
      myPlane= currentRecord->getDouble("ZPLANE");
      myRmin = currentRecord->getDouble("RMIN");
      myRmax = currentRecord->getDouble("RMAX");
      if ( ind == 0 || ind == 1 ) myRmin = 47.;
      cryoMotherShape->addPlane(myPlane,
                                myRmin,
                                myRmax);
      if(ind==0)
  ...
  if(m_pAccessSvc->getChildTag("HGTD",detectorKey, detectorNode)!="") {
    // DB related stuff first
...
//      double zposMod = (*itModerator)->getDouble("ZPOS")*CLHEP::mm;
      ...
      pvMM->add(pvDead1);      
      // Active material
      GeoLogVol* lvSilicon1 = new GeoLogVol("HGTDsi1",solidSilicon, matSi);
      ...
      pvMM->add(pvDead2);      
      // Active material
      GeoLogVol* lvSilicon2 = new GeoLogVol("HGTDsi2",solidSilicon, matSi);
      ...
      pvMM->add(pvDead3);      
      // Active material
      GeoLogVol* lvSilicon3 = new GeoLogVol("HGTDsi3",solidSilicon, matSi);
      ...
      pvMM->add(pvDead4);      
      // Active material
      GeoLogVol* lvSilicon4 = new GeoLogVol("HGTDsi4",solidSilicon, matSi);
