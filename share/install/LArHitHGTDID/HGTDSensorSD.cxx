...
G4bool HGTDSensorSD::ProcessHits(G4Step* aStep, G4TouchableHistory* /*ROhist*/)
{
  ...
  int sampling = -1;
  if(detname_layer == "LArMgr::HGTDsi1" ) sampling=0;
  else if(detname_layer == "LArMgr::HGTDsi2" ) sampling=1;
  else if(detname_layer == "LArMgr::HGTDsi3" ) sampling=2;
  else if(detname_layer == "LArMgr::HGTDsi4" ) sampling=3;
  else {
...
void HGTDSensorSD::EndOfAthenaEvent( LArHitContainer * hitContainer )
{
  ...
  for(const auto i : m_timeBins) {
    ...
    for(auto hit : *hitSet){
      ...
      hit->finalize();
      //      std::cout << "DIRK " << hit->cellID().getString() << " " << m_hgtdID->channel_hash(hit->cellID()) << " " << hit->energy() << " "<< hit->time() << std::endl;
