...
ISF::SimHitSvc::SimHitSvc(const std::string& name,ISvcLocator* svc) :
  ...
  m_hgtdHits(nullptr),
  m_hgtdHitCollectionName("HGTDHits"),
  m_simulateMS(true),
  m_mdtHits(nullptr),
  ...
  m_fastSimTool("FastSimulationMasterTool")
{
  ...
  declareProperty("HGTD_HitCollection",  m_hgtdHitCollectionName );
...
StatusCode ISF::SimHitSvc::initializeEvent()
{
  ...
  m_hgtdHits = m_hitCollectionHelper->RetrieveNonconstCollection<SiHitCollection>(m_hgtdHitCollectionName); 
...
StatusCode ISF::SimHitSvc::releaseEvent()
{
  ...
  m_hitCollectionHelper->SetConstCollection<SiHitCollection>(m_hgtdHits);
  m_hgtdHits=0;
